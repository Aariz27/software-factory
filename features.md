...
- [ ] 11. **Onboarding** (`/onboard` extension) — detect which CLIs are logged in, list the models each one exposes (`claude`, `codex`, `agy models`, `ollama list`), and ask the user which model runs each `/` command (plus a backup per command); write the choices to `blueprint/config.json`. No model cards — the user chooses.
